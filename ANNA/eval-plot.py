            ...
            with codecs.open("results/" + mname, "w", "utf-8") as fout:
                ...
                for i in range(len(X) // hp.batch_size):
                    ...
                    for j in range(hp.maxlen):
                        _preds, enc = sess.run([g.preds, g.enc],{g.x: x, g.y: y, g.y_decoder_input: preds})
                        preds[:, j] = _preds[:, j]
                        enc = np.reshape(enc, [hp.batch_size, hp.maxlen, hp.hidden_units])

                    np.save('en_array', enc)

                    for j in range(len(y)):
                        ppl = sess.run(g.ppl,{g.x: x, g.y: y, g.y_decoder_input: y})
                        ppls.append(ppl)
